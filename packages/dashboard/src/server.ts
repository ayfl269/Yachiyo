  ...
  private ensureDefaultUser(): void {
    try {
      ...
      if (!countRow || countRow.count === 0) {
        ...
        if (autogenerated) {
          // Log a one-time hint so the operator knows to check the process
          // output. The actual password is masked to prevent clear-text
          // leakage in shared logs; the operator must retrieve it from the
          // process environment or the startup transcript securely.
          console.warn(
            `[DashboardServer] No strong DASHBOARD_DEFAULT_PASSWORD provided; ` +
            `generated a random one-time password for user "${defaultUser}". ` +
            `Password hint: ${defaultPass.slice(0, 2)}${"*".repeat(Math.max(0, defaultPass.length - 4))}${defaultPass.slice(-2)}\n` +
            `Please log in immediately and change it. ` +
