  ...
  private ensureDefaultUser(): void {
    try {
      ...
      if (!countRow || countRow.count === 0) {
        ...
        if (autogenerated) {
          // Log the one-time password so the operator can complete first login.
          // Force-change on first login guarantees this credential is short-lived.
          console.warn(
            `[DashboardServer] No strong DASHBOARD_DEFAULT_PASSWORD provided; ` +
            `generated a random one-time password for user "${defaultUser}": ${defaultPass}\n` +
